...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        if sr_ratio > 1:
            ...
            self.norm = DynamicTanh(dim, channels_last=True)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.norm1 = DynamicLinear(dim, channels_last=True)
        self.attn = Attention(
            dim,
        ...
        self.norm2 = DynamicLinear(dim, channels_last=True)
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            ...
            # DynamicTanh는 자체적으로 초기화되므로 건드리지 않음
            pass
        elif isinstance(m, nn.LayerNorm):
            nn.init.constant_(m.bias, 0)
            nn.init.constant_(m.weight, 1.0)
...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        ...
        self.norm = DynamicTanh(embed_dim, channels_last=True)
