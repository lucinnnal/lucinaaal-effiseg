...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        ...
        nn.Conv2d(in_chans, embed_dim // 4, kernel_size=1, stride=1, bias=False),  # 1x1 conv (compression)
        nn.BatchNorm2d(embed_dim // 4),
        nn.ReLU(inplace=True),

        nn.Conv2d(embed_dim // 4, embed_dim // 4, kernel_size=patch_size, stride=stride,
                padding=(patch_size[0] // 2, patch_size[1] // 2), bias=False),     # 3x3 conv (feature extraction)
        nn.BatchNorm2d(embed_dim // 4),
        nn.ReLU(inplace=True),

        nn.Conv2d(embed_dim // 4, embed_dim, kernel_size=1, stride=1, bias=False),  # 1x1 conv (expansion)
...
class MixVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=20, embed_dims=[64, 128, 256, 512],
    ...
    def forward(self, x, is_feat=True):
        ...
        _c4 = resize(_c4, size=x[0].size()[2:],mode='bilinear',align_corners=False)

        _c3 = self.linear_c3(x[2]).permute(0,2,1).reshape(n, -1, x[2].shape[2], x[2].shape[3])
        _c3 = resize(_c3, size=x[0].size()[2:],mode='bilinear',align_corners=False)

        _c2 = self.linear_c2(x[1]).permute(0,2,1).reshape(n, -1, x[1].shape[2], x[1].shape[3])
        _c2 = resize(_c2, size=x[0].size()[2:],mode='bilinear',align_corners=False)
...
class mit_b0(MixVisionTransformer):
    def __init__(self, **kwargs):
        super(mit_b0, self).__init__(
            ...
            drop_rate=0.0, drop_path_rate=0.1, decoderembed_dim=256)
