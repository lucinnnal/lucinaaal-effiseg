...
class iouEval:
    ...
    def addBatch(self, x, y):   #x=preds, y=targets => model의 logit의 각 픽셀 별로 argmax를 하여 pred로 전달해야함.
        ...
        # 각 픽셀별 예측 클래스/정답 클래스의 id에 대해 one-hot encoding을 수행하기

        """
        => tensor.scatter_(dim, index, src)
        dim: 값을 흩뿌릴 축 (여기서는 1, 즉 channel 축)
	    index: 위치 인덱스 (여기서는 y)
	    src: 채워 넣을 값 (여기서는 1)
	    scatter_는 in-place 연산입니다. 즉, y_onehot을 직접 수정합니다.
        """
